<template>
  <div>
    <h2 id="page-heading" data-cy="BlogHeading">
      <span v-text="t$('demoappApp.demoappBlog.home.title')" id="blog-heading"></span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="t$('demoappApp.demoappBlog.home.refreshListLabel')"></span>
        </button>
        <router-link :to="{ name: 'BlogCreate' }" custom v-slot="{ navigate }">
          <button @click="navigate" id="jh-create-entity" data-cy="entityCreateButton" class="btn btn-primary jh-create-entity create-blog">
            ...
            <span v-text="t$('demoappApp.demoappBlog.home.createLabel')"></span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && blogs && blogs.length === 0">
      <span v-text="t$('demoappApp.demoappBlog.home.notFound')"></span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="blogs">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="t$('demoappApp.demoappBlog.name')"></span></th>
            <th scope="row"><span v-text="t$('demoappApp.demoappBlog.handle')"></span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #modal-title>
        <span id="demoappApp.demoappBlog.delete.question" data-cy="blogDeleteDialogHeading" v-text="t$('entity.delete.title')"></span>
      </template>
      <div class="modal-body">
        <p id="jhi-delete-blog-heading" v-text="t$('demoappApp.demoappBlog.delete.question', { id: removeId })"></p>
      </div>
